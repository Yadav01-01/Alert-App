...
//            }
    ...
    override suspend fun getInAppNotification(): Flow<NetworkResult<MutableList<Alert>>> = flow {
        try {
            apiInterface.getInAppNotification().apply {
                if (isSuccessful) {
                    body()?.let { resp ->
                        if (resp.has("status") && resp.get("status").asBoolean) {
                            val gson = Gson()
                            ...
                            arr.forEach {
                                val obj = it.asJsonObject
                                val response = gson.fromJson(obj, Alert::class.java)
                                list.add(response)
                            }
    ...
    override suspend fun setAlertWrongPath(journeyId: String): Flow<NetworkResult<String>> = flow {
        try {
            apiInterface.setAlertWrongPath(journeyId).apply {
                if (isSuccessful) {
                    body()?.let { resp ->
                        if (resp.has("status") && resp.get("status").asBoolean) {
                            emit(NetworkResult.Success(resp.get("message").asString))
